...
compilePred file = fmap (cond (== ExitSuccess) true false . p1) $ shellStrict command empty
  where
    command = "stack exec ghc " <> file <> " -- -e ':t id'"

runProp :: Text -> Prop -> IO Text
runProp file (Prop prop _ qcCommand) =
  let command =
        if isNothing qcCommand
...
typeProp file (Prop prop _ _) = fmap p2 $ shellStrict command empty
  where
    command = "stack exec ghc " <> file <> " -- -e ':t " <> prop <> "'"

failProps :: Text -> [Prop] -> IO [Text]
failProps file = cataList (either epsilon testProp)
  where
...
failTypes file = cataList (either epsilon testType)
  where
    epsilon = const (return [])
