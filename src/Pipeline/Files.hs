...
import Cp (false)
import Pipeline.Utils (insideZip, insideZipStack, procError, tFile, tFileStack)
import Turtle
import Prelude hiding (FilePath)

lsZips :: MonadIO a => FilePath -> a ([FilePath], [FilePath])
lsZips file = fold (ls file) (liftA2 (,) zipFold zipStackFold)
  where
...
unzipFileGeneric iZip outDir file = shell command empty >>= procError
  where
